...
class EventController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Log::debug('Validating request data', ['request' => $request->all()]);
        // Validasi data event utama (tidak termasuk tiket)
        $validated = $request->validate([
            'name' => 'required|string|max:70',
            'description' => 'required|string',
            'date' => 'required|date_format:d/m/Y',
        ...
        $event = Event::create([
            ...
            'location' => $validated['location'],
            'image_path' => $imagePath,
            'category_id' => $validated['category_id'],  // Pastikan category_id sesuai dengan request
            'user_id' => Auth::user()->id
        ]);
        Log::debug('Event created successfully', ['event' => $event]);
    
        Log::debug('Creating ticket records');
        // Langsung mengambil data tiket dari request dan menyimpannya
        foreach ($request->input('tickets', []) as $ticket) {
            ...
            $event->tickets()->create([
                ...
                'price' => $ticket['price'],
                'quota' => $ticket['quota'],
                'description' => $ticket['description'] ?? null, // Optional jika tidak ada deskripsi
            ]);
        }
        Log::debug('All tickets created successfully');
